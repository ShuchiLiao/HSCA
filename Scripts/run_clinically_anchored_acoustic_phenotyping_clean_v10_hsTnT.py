...
CLINICAL_PANELS: Dict[str, List[str]] = {
    "functional_impairment_hf_burden": ["EF_Teich", "NTproBNP", "NYHA", "hsTnT"],
...
def build_position_endpoint_configs(positions: Sequence[str]):
    """Return full, only-position, and leave-one-position configurations."""
    positions = [p.strip().upper() for p in positions if str(p).strip()]
    configs = [("full_position_concat", positions, "concat", "full")]
    configs += [(f"only_{p}", [p], "single", "only_one_site") for p in positions]
    configs += [(f"leave_{p}", [q for q in positions if q != p], "concat", "leave_one_site") for p in positions]
    return configs


def run_position_endpoint_validation(
    clinical: pd.DataFrame,
    patient_ids: Sequence[str],
    args,
) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    """Leave-endpoint-out endpoint validation under site configurations."""
    if getattr(args, "skip_position_endpoint_validation", False):
    ...
    clinical_by_pid = clinical.set_index(PATIENT_ID_COL, drop=False)
    positions = [p.strip().upper() for p in args.positions.split(",") if p.strip()]
    configs = build_position_endpoint_configs(positions)

    summary_rows, pred_rows, info_rows = [], [], []
    for name, pos_list, mode, analysis_type in configs:
        Xc, pids = build_position_mode_X(Xpos, meta, patient_ids, mode=mode, positions_to_use=pos_list)
        if len(pids) < args.min_panel_n:
            log(f"Position endpoint validation skipped: {name}, n={len(pids)}")
            continue
        Cl = clinical_by_pid.loc[pids].reset_index(drop=True)
        log(f"Position endpoint validation: {name}, positions={','.join(pos_list)}, n={len(pids)}")
        try:
            ep_summary, ep_pred, ep_info = run_endpoint_validation(Xc, Cl, pids, args)
            for df in [ep_summary, ep_pred, ep_info]:
                if len(df):
                    df["position_analysis"] = name
                    df["site_analysis_type"] = analysis_type
                    df["included_positions"] = ",".join(pos_list)
                    df["n_positions_used"] = len(pos_list)
                    df["position_mode"] = mode
            if len(ep_summary):
                summary_rows.append(ep_summary)
            if len(ep_pred):
                pred_rows.append(ep_pred)
            if len(ep_info):
                info_rows.append(ep_info)
        except Exception as e:
            log(f"Position endpoint validation failed: {name}: {e}")
            info_rows.append(pd.DataFrame([{
                "position_analysis": name,
                "site_analysis_type": analysis_type,
                "included_positions": ",".join(pos_list),
                "n_positions_used": len(pos_list),
                "position_mode": mode,
                "status": f"failed: {e}",
    ...
    info = pd.concat(info_rows, ignore_index=True) if info_rows else pd.DataFrame()
    return summary, pred, info


def plot_position_endpoint_heatmap(
    summary_df: pd.DataFrame,
    out_path: Path,
    site_analysis_type: str,
    axis_feature_set: str = "axis1",
    metric: str = "auroc",
) -> None:
    """Heatmap of endpoint AUROC across only-site or leave-one-site settings."""
    if summary_df is None or len(summary_df) == 0:
        return
    required = {"endpoint", "position_analysis", "site_analysis_type", "axis_feature_set", metric}
    if not required.issubset(summary_df.columns):
        return
    d = summary_df[
        (summary_df["site_analysis_type"] == site_analysis_type)
        & (summary_df["axis_feature_set"].astype(str) == axis_feature_set)
    ].copy()
    d = d[np.isfinite(d[metric])]
    if len(d) == 0:
        return

    endpoint_order = ["EF_lt_40", "NTproBNP_ge_900", "NYHA_ge_3", "LVEDD_dilated", "LA_ge_40"]
    endpoint_order = [e for e in endpoint_order if e in set(d["endpoint"])]
    if site_analysis_type == "only_one_site":
        config_order = ["only_A", "only_E", "only_M", "only_P", "only_T"]
        title = "Endpoint AUROC by single auscultation site"
        xlabel = "Single-site acoustic input"
    elif site_analysis_type == "leave_one_site":
        config_order = ["leave_A", "leave_E", "leave_M", "leave_P", "leave_T"]
        title = "Endpoint AUROC after leaving out one auscultation site"
        xlabel = "Site configuration"
    else:
        config_order = sorted(d["position_analysis"].unique())
        title = f"Endpoint AUROC by {site_analysis_type}"
        xlabel = "Site configuration"
    config_order = [c for c in config_order if c in set(d["position_analysis"])]
    if not endpoint_order or not config_order:
        return

    mat = d.pivot_table(index="endpoint", columns="position_analysis", values=metric, aggfunc="first")
    mat = mat.reindex(index=endpoint_order, columns=config_order)
    ...
    plt.figure(figsize=(9.4, max(4.6, 0.65 * len(endpoint_order))))
    ax = plt.gca()
    vmax = max(0.85, float(np.nanmax(mat.values)) if np.isfinite(mat.values).any() else 0.85)
    if HAS_SEABORN:
        sns.heatmap(
            mat.astype(float),
            annot=True,
        ...
        ax.set_xticklabels([labels.get(c, c) for c in config_order], rotation=0)
        ax.set_yticklabels([ylabels.get(e, e) for e in endpoint_order], rotation=0)
    else:
        im = ax.imshow(mat.values.astype(float), aspect="auto", vmin=0.5, vmax=vmax)
        plt.colorbar(im, ax=ax, label="AUROC")
        ax.set_xticks(np.arange(len(config_order)))
        ax.set_xticklabels([labels.get(c, c) for c in config_order], rotation=0)
        ax.set_yticks(np.arange(len(endpoint_order)))
        ax.set_yticklabels([ylabels.get(e, e) for e in endpoint_order])
    ...
    ax.set_ylabel("")
    ax.set_title(f"{title} ({'axis 1' if axis_feature_set == 'axis1' else axis_feature_set})")
    savefig(out_path)


def plot_position_endpoint_forest(
    summary_df: pd.DataFrame,
    out_path: Path,
    axis_feature_set: str = "axis1",
) -> None:
    """Compact forest/dot plot comparing full, only-site, and leave-one-site endpoint AUROCs."""
    if summary_df is None or len(summary_df) == 0:
        return
    required = {"endpoint", "position_analysis", "site_analysis_type", "axis_feature_set", "auroc"}
    if not required.issubset(summary_df.columns):
        return
    d = summary_df[summary_df["axis_feature_set"].astype(str) == axis_feature_set].copy()
    d = d[np.isfinite(d["auroc"])]
    if len(d) == 0:
        return
    endpoint_order = ["EF_lt_40", "NTproBNP_ge_900", "NYHA_ge_3", "LVEDD_dilated", "LA_ge_40"]
    ...
    plt.figure(figsize=(11.5, max(5.0, 0.8 * len(endpoint_order))))
    ax = plt.gca()
    y_base = np.arange(len(endpoint_order))
    offsets = np.linspace(-0.28, 0.28, len(config_order)) if len(config_order) > 1 else np.array([0.0])
    cmap = plt.get_cmap("tab20")
    for j, cfg in enumerate(config_order):
        sub = d[d["position_analysis"] == cfg]
        xs, ys, lo_err, hi_err = [], [], [], []
        for i, ep in enumerate(endpoint_order):
            hit = sub[sub["endpoint"] == ep]
            if len(hit) == 0:
            ...
            lo = float(row["auroc_ci95_low"]) if "auroc_ci95_low" in row and pd.notna(row["auroc_ci95_low"]) else np.nan
            hi = float(row["auroc_ci95_high"]) if "auroc_ci95_high" in row and pd.notna(row["auroc_ci95_high"]) else np.nan
            xs.append(x); ys.append(i + offsets[j])
            lo_err.append(x - lo if np.isfinite(lo) else 0)
            hi_err.append(hi - x if np.isfinite(hi) else 0)
        if xs:
            ax.errorbar(xs, ys, xerr=[lo_err, hi_err], fmt="o", capsize=2.5, ms=4.5, lw=1.2, color=cmap(j % 20), label=labels.get(cfg, cfg))
    ax.axvline(0.5, color="black", linestyle="--", lw=1)
    ax.set_yticks(y_base)
    ax.set_yticklabels([endpoint_labels.get(e, e) for e in endpoint_order])
    ax.set_xlim(0.4, 1.0)
    ax.set_xlabel("AUROC")
    ax.set_title(f"Endpoint AUROC across auscultation-site configurations ({'axis 1' if axis_feature_set == 'axis1' else axis_feature_set})")
...
# =============================================================================
# Patient-level all-clinical CCA axis score export
# =============================================================================

def export_allclinical_patient_axis_scores(scores_all: pd.DataFrame, clinical: pd.DataFrame, out_path: Path) -> pd.DataFrame:
    """Export patient-level all-clinical CCA axis scores for ranked score plots.

    The primary table contains the out-of-fold all-clinical, unadjusted CCA scores:
      - cca_acoustic_axis1 / axis2
      - cca_clinical_axis1 / axis2
      - rank, empirical quantile, percentile, and quartile of acoustic axis 1
      - fixed clinical variables and key endpoint labels

    These scores are generated only from held-out folds in run_oof_cca(), so they
    are suitable for downstream empirical quantile / ranked score plots.
    """
    if scores_all is None or len(scores_all) == 0:
        return pd.DataFrame()
    required = {PATIENT_ID_COL, "panel", "adjustment", "cca_acoustic_axis1"}
    if not required.issubset(scores_all.columns):
        return pd.DataFrame()

    d = scores_all[
        scores_all["panel"].astype(str).eq("all_clinical")
        & scores_all["adjustment"].astype(str).eq("none")
    ].copy()
    if len(d) == 0:
        return pd.DataFrame()
    ...
    for c in ["cca_acoustic_axis2", "cca_clinical_axis2"]:
        if c in d.columns:
            keep_score_cols.append(c)
    d = d[keep_score_cols].copy()

    d = d[np.isfinite(d["cca_acoustic_axis1"])].copy()
    d = d.sort_values("cca_acoustic_axis1", ascending=True).reset_index(drop=True)
    n = len(d)
    d["axis1_rank_low_to_high"] = np.arange(1, n + 1)
    d["axis1_empirical_quantile"] = (d["axis1_rank_low_to_high"] - 0.5) / n if n else np.nan
    d["axis1_percentile"] = 100.0 * d["axis1_empirical_quantile"]
    if n >= 4:
        d["axis1_quartile"] = pd.qcut(
            d["axis1_rank_low_to_high"],
            q=4,
            labels=["Q1_lowest", "Q2", "Q3", "Q4_highest"],
        ).astype(str)
    else:
        d["axis1_quartile"] = np.nan

    clinical_cols = [PATIENT_ID_COL] + ALL_CLINICAL_VARS + ["age_years", "sex_male", "heart_rate"]
    ctab = clinical[[c for c in clinical_cols if c in clinical.columns]].copy()
    out = d.merge(ctab, on=PATIENT_ID_COL, how="left")

    # Add key endpoint labels for convenient coloring/annotation in ranked plots.
    for endpoint in ["EF_lt_40", "NTproBNP_ge_900", "NYHA_ge_3", "LA_ge_40", "LVEDD_dilated"]:
        try:
            y, source, rule = make_endpoint(out, endpoint)
            out[endpoint] = y
            out[f"{endpoint}_source"] = source
            out[f"{endpoint}_rule"] = rule
        except Exception:
            out[endpoint] = np.nan

    out.to_csv(out_path, index=False, encoding="utf-8-sig")
    log(f"Saved patient-level all-clinical CCA axis score table: {out_path}")
    return out


def plot_allclinical_axis1_ranked_score(score_table: pd.DataFrame, out_path: Path) -> None:
    """Ranked score / empirical quantile plot for all-clinical acoustic CCA axis 1."""
    if score_table is None or len(score_table) == 0:
        return
    d = score_table.copy()
    if not {"axis1_empirical_quantile", "cca_acoustic_axis1"}.issubset(d.columns):
        return
    ...
    ax = plt.gca()
    color = "#0072B2"
    ax.plot(d["axis1_empirical_quantile"], d["cca_acoustic_axis1"], color=color, lw=2.2)
    ax.scatter(d["axis1_empirical_quantile"], d["cca_acoustic_axis1"], s=13, color=color, alpha=0.45, edgecolor="none")
    ax.axhline(0, color="black", lw=1, alpha=0.75)
    for q in [0.25, 0.50, 0.75]:
        ax.axvline(q, color="0.70", lw=1, linestyle="--")
    ax.set_xlabel("Empirical quantile of all-clinical acoustic CCA axis 1")
    ax.set_ylabel("All-clinical acoustic CCA axis 1 score")
    ax.set_title("Ranked patient scores on the all-clinical acoustic CCA axis 1")
    savefig(out_path)
...
def main():
    ...
    allclinical_axis_score_table = export_allclinical_patient_axis_scores(
        scores_all,
        clinical,
        table_dir / "allclinical_patient_axis_scores_ranked.csv",
    )
    plot_allclinical_axis1_ranked_score(
        allclinical_axis_score_table,
        fig_dir / "allclinical_axis1_ranked_score_plot.png",
    )
    ...
    # Endpoint AUROC sensitivity by auscultation-site configuration.
    # This uses the same leave-endpoint-out CCA validation logic as the main endpoint analysis,
    # but rebuilds X from position-level embeddings for only-site and leave-one-site inputs.
    position_endpoint_summary, position_endpoint_pred, position_endpoint_info = run_position_endpoint_validation(
        clinical, patient_ids, args
    )
    position_endpoint_summary.to_csv(table_dir / "position_endpoint_validation_summary.csv", index=False, encoding="utf-8-sig")
    position_endpoint_pred.to_csv(table_dir / "position_endpoint_validation_predictions.csv", index=False, encoding="utf-8-sig")
    position_endpoint_info.to_csv(table_dir / "position_endpoint_validation_info.csv", index=False, encoding="utf-8-sig")
    plot_position_endpoint_heatmap(
        position_endpoint_summary,
        fig_dir / "endpoint_auroc_by_only_site_axis1_heatmap.png",
        site_analysis_type="only_one_site",
        axis_feature_set="axis1",
    )
    plot_position_endpoint_heatmap(
        position_endpoint_summary,
        fig_dir / "endpoint_auroc_by_leave_one_site_axis1_heatmap.png",
        site_analysis_type="leave_one_site",
        axis_feature_set="axis1",
    )
    plot_position_endpoint_heatmap(
        position_endpoint_summary,
        fig_dir / "endpoint_auroc_by_only_site_axis12_heatmap.png",
        site_analysis_type="only_one_site",
        axis_feature_set="axis1_2",
    )
    plot_position_endpoint_heatmap(
        position_endpoint_summary,
        fig_dir / "endpoint_auroc_by_leave_one_site_axis12_heatmap.png",
        site_analysis_type="leave_one_site",
        axis_feature_set="axis1_2",
    )
    plot_position_endpoint_forest(
        position_endpoint_summary,
        fig_dir / "endpoint_auroc_by_site_configuration_axis1_forest.png",
        axis_feature_set="axis1",
    )
