...
def _evaluate_retrieval_predictions(y_true: pd.Series, pred: pd.Series, prob: pd.Series, spec: VariableSpec) -> Dict[str, float]:
    ...
    if spec.var_type in {"continuous", "ordinal"}:
        ...
        yp_num = yp_num[mask2]
        if len(yt_num) == 0:
            return {"n_eval": 0, "mae": np.nan, "rmse": np.nan, "spearman": np.nan}
        ...
        return {"n_eval": int(len(yt_num)), "mae": mae, "rmse": rmse, "spearman": rho}
...
def run_retrieval_validation(
    ...
    for spec in target_specs:
        ...
        for k in knn_list:
            ...
            summary_rows.append({"target": spec.clean_name, "group": spec.group, "target_type": spec.var_type, "k": int(k), **metrics})

            for pid, truth, pred_val, pred_prob, top1, conf in zip(patient_ids, y.tolist(), pred_values, pred_probs, top1_ids, mean_topk_dist):
                prediction_rows.append({
                    ...
                    "top1_neighbor_id": top1,
                    "mean_topk_distance": conf,
                })
