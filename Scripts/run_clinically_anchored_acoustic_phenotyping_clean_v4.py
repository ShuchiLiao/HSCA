...
def run_repeated_endpoint_validation(
    ...
    fitted on the training fold only, and a logistic classifier is trained on
    training-fold acoustic CCA axes 1+2.
    """
    n_repeats = int(args.n_random_split_repeats)
    endpoints = ["EF_lt_40", "NTproBNP_ge_300", "NYHA_ge_3", "LA_ge_40", "LVEDD_dilated"]
    rows = []
    ...
    if len(values):
        ok = values["status"].eq("ok")
        for endpoint, sub in values[ok].groupby("endpoint"):
