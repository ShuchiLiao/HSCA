...
def plot_figureS05_endpoint_threshold_heatmap(threshold_df: pd.DataFrame, out_path: Path) -> None:
    ...
    plt.figure(figsize=(5.8, max(4.8, 0.46 * len(pivot.index))))
    ax = plt.gca()
    if HAS_SEABORN:
        sns.heatmap(
            pivot,
...
def plot_figureS06_repeated_endpoint_auroc_by_split(repeated_endpoint_df: pd.DataFrame, out_path: Path) -> None:
    """Boxplot of repeated StratifiedKFold endpoint AUROC for EF, BNP, NYHA, and LVEDD.

    Each point is one repeated CV split. The plot uses Axis 1 by default to match
    the main manuscript line. If the table lacks n_axis_features, it is treated as
    Axis 1.
    """
    ...
    # Keep Axis 1 only for the main repeated endpoint validation plot.
    if "n_axis_features" in d.columns:
        d["n_axis_features"] = pd.to_numeric(d["n_axis_features"], errors="coerce").fillna(1).astype(int)
    ...
    # Keep the four main manuscript endpoints. Support both BNP>=900 and older BNP>=300 names.
    endpoint_order = ["EF_lt_40", "NTproBNP_ge_900", "NTproBNP_ge_300", "NYHA_ge_3", "LVEDD_dilated"]
    keep_endpoints = set(endpoint_order)
    d = d[d["endpoint"].astype(str).isin(keep_endpoints)].copy()
    if len(d) == 0:
        return

    # If both BNP>=900 and BNP>=300 exist, prefer BNP>=900.
    ...
    ax = plt.gca()
    palette = {
        _endpoint_pretty("EF_lt_40"): "#009E73",
        _endpoint_pretty("NTproBNP_ge_900"): "#CC79A7",
        _endpoint_pretty("NTproBNP_ge_300"): "#CC79A7",
        _endpoint_pretty("NYHA_ge_3"): "#56B4E9",
        _endpoint_pretty("LVEDD_dilated"): "#D55E00",
    }
    ...
    if HAS_SEABORN:
        ...
        for i, vals in enumerate(groups, start=1):
            ax.scatter(i + rng.uniform(-0.12, 0.12, len(vals)), vals, color="black", alpha=0.58, s=18)

    ax.axhline(0.5, color="black", linestyle="--", lw=1.05, alpha=0.75)
    ax.set_ylim(0.45, 0.95)
    ax.set_xlabel("")
    ax.set_ylabel("AUROC across repeated CV splits")
    ax.set_title("Repeated-CV endpoint validation AUROC (Axis 1)")
    ax.tick_params(axis="x", rotation=18)
    savefig(out_path)


# =============================================================================
# Style overrides and final publication figure refinements
# =============================================================================
...
def plot_figure02_axis1_variable_lollipop(assoc_df: pd.DataFrame, out_path: Path) -> None:
    ...
    plt.figure(figsize=(10.6, max(6.4, 0.55 * len(d))))
    ax = plt.gca()
    y = np.arange(len(d))
    x = d["spearman_axis_variable"].to_numpy(float)
    lo = d["spearman_ci95_low"].to_numpy(float) if "spearman_ci95_low" in d.columns else np.full(len(d), np.nan)
...
def plot_endpoint_roc(pred_df: pd.DataFrame, summary_df: pd.DataFrame, endpoint: str, pretty: str, out_path: Path, args) -> None:
    ...
    plt.figure(figsize=(6.6, 6.1))
    ax = plt.gca()
    ax.plot([0, 1], [0, 1], linestyle="--", color="black", lw=1.2, label="Chance")
    if np.isfinite(lo).any():
...
def plot_figure10_site_robustness_contribution(
    ...
    handles = [
        plt.Line2D([0], [0], marker="o", color="w", markerfacecolor=colors["Only-site"], markeredgecolor="black", label="Only-site", markersize=12),
        plt.Line2D([0], [0], marker="o", color="w", markerfacecolor=colors["Leave-one-site"], markeredgecolor="black", label="Leave-one-site", markersize=12),
...
def plot_figure11_ranked_axis_score(axis_rank_df: pd.DataFrame, axis_scores: pd.DataFrame, out_path: Path) -> None:
    ...
    if color_col:
        ...
        ax.scatter(d["axis1_empirical_quantile"], d["cca_acoustic_axis1"], color="#0072B2", s=36, alpha=0.78, edgecolor="none")
    ax.axhline(0, color="black", lw=1, alpha=0.72)
    for q in [0.25, 0.50, 0.75]:
        ax.axvline(q, color="0.72", linestyle="--", lw=1)
    ax.set_xlabel("Empirical quantile of all-clinical acoustic CCA axis 1")
...
def plot_figureS02_negative_controls(ctrl_df: pd.DataFrame, summary_df: pd.DataFrame, out_path: Path) -> None:
    ...
    ax.set_xlabel("OOF Spearman correlation under control")
    ax.set_ylabel("Count")
    ax.legend(frameon=False, fontsize=21)
    savefig(out_path)
