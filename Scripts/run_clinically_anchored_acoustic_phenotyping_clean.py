...
def clinical_gradient_summary(patient_long: pd.DataFrame) -> pd.DataFrame:
    rows = []
    for (panel, adj, comp, var, group), sub in patient_long.groupby(["panel", "adjustment", "component", "variable", "axis_group"]):
...
def make_endpoint(clinical: pd.DataFrame, name: str) -> Tuple[pd.Series, str, str]:
    ...
    if name == "NTproBNP_ge_300":
        v = clinical["NTproBNP"]
        threshold = np.log1p(300.0) if np.nanmax(v.values) <= 20 else 300.0
        y.loc[v.notna()] = (v.loc[v.notna()] >= threshold).astype(float)
        return y, "NTproBNP", f"NTproBNP >= {threshold:.4g} (NT-proBNP >=300 if log1p transformed)"
    if name == "NYHA_ge_3":
        v = clinical["NYHA"]
...
def run_endpoint_validation(X: np.ndarray, clinical: pd.DataFrame, patient_ids: Sequence[str], args):
    endpoints = ["EF_lt_40", "NTproBNP_ge_300", "NYHA_ge_3", "LA_ge_40", "LVEDD_dilated"]
    ...
    for endpoint in endpoints:
        ...
        info = {"endpoint": endpoint, "source_column": source, "rule": rule, "n": int(valid.sum()), "n_positive": int(y_all.sum()) if len(y_all) else 0, "positive_rate": float(y_all.mean()) if len(y_all) else np.nan}
        ...
        for pid, yt, yp, f in zip(pids, y_all, prob, folds):
            pred_rows.append({PATIENT_ID_COL: pid, "endpoint": endpoint, "y_true": int(yt), "y_prob": float(yp), "fold": int(f), "panel_type": panel_type})
...
def plot_alignment_forest(df: pd.DataFrame, out_path: Path) -> None:
    ...
    plt.figure(figsize=(10, max(5.5, 0.48 * len(d))))
    ax = plt.gca()
    y = np.arange(len(d))
...
def plot_axis_association_forest(df: pd.DataFrame, panel: str, out_path: Path, component: int = 1) -> None:
    ...
    plt.figure(figsize=(9, max(4.5, 0.45 * len(d))))
    ax = plt.gca()
    y = np.arange(len(d))
    ...
    ax.set_title(f"Axis interpretation: {panel} axis {component}")
    savefig(out_path)


def plot_gradient_box_points(patient_long: pd.DataFrame, panel: str, out_path: Path, component: int = 1, max_vars: int = 8) -> None:
    d = patient_long[(patient_long["panel"] == panel) & (patient_long["adjustment"] == "none") & (patient_long["component"] == component)].copy()
    if len(d) == 0:
        return
    # Show variables in panel order.
    vars_order = [v for v in CLINICAL_PANELS.get(panel, ALL_CLINICAL_VARS) if v in set(d["variable"])]
    vars_order = vars_order[:max_vars]
    n = len(vars_order)
    if n == 0:
        return
    ...
    for ax, var in zip(axes.ravel(), vars_order):
        ...
        sub["axis_group"] = pd.Categorical(sub["axis_group"], categories=[f"Q{i+1}" for i in range(4)], ordered=True)
        if HAS_SEABORN:
            sns.boxplot(data=sub, x="axis_group", y="value", ax=ax, color="white", showfliers=False, linewidth=1.3)
            sns.stripplot(data=sub, x="axis_group", y="value", ax=ax, size=2.3, alpha=0.28, jitter=0.25)
        else:
            groups = [sub.loc[sub["axis_group"] == f"Q{i+1}", "value"].dropna().to_numpy(float) for i in range(4)]
            ax.boxplot(groups, labels=[f"Q{i+1}" for i in range(4)], showfliers=False)
            for i, vals in enumerate(groups, start=1):
                ax.scatter(i + rng.uniform(-0.22, 0.22, len(vals)), vals, s=8, alpha=0.25)
        rho, _, _ = safe_spearman(sub["axis_score"], sub["value"])
        ax.set_title(f"{PRETTY_LABELS.get(var, var)}" + (f" (ρ={rho:.2f})" if np.isfinite(rho) else ""))
    ...
    for ax in axes.ravel()[n:]:
        ax.axis("off")
    fig.suptitle(f"Clinical gradient across {panel} acoustic axis {component}", y=1.02, fontsize=16)
    savefig(out_path)
...
def main():
    ...
    for panel, vars_ in panels.items():
        ...
        if panel in CLINICAL_PANELS:
            ...
            plot_gradient_box_points(gpat, panel, fig_dir / f"{clean_filename(panel)}_axis1_gradient_box_point.png")
