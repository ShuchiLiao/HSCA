...
def detect_clinical_concepts(num_df: pd.DataFrame, args) -> pd.DataFrame:
    ...
    Exact cleaned-table column names are tried first. This is important for columns
    such as EF_Teich and LA_mm, which do not match strict regex patterns like ^ef$
    or ^la$. Regex matching is kept only as a fallback for future tables.
    """
    ...
    for cc in CONCEPTS:
        ...
        # 1) Prefer exact cleaned-table columns.
        for col in EXACT_CONCEPT_COLUMNS.get(cc.concept, []):
            if col in cols and col not in used_cols and col not in matches:
                matches.append(col)

        # 2) Fall back to regex matching.
        for col in cols:
            if col in used_cols or col in matches:
                continue
        ...
        for col in matches:
            ...
            row = {
                ...
                "matched_by_exact_column": col in EXACT_CONCEPT_COLUMNS.get(cc.concept, []),
            }
            cand.append(row)
        ...
        cand_sorted = sorted(
            ...
            key=lambda r: (
                ...
                0 if r.get("matched_by_exact_column", False) else 1,
...
def find_covariate_columns(num_df: pd.DataFrame) -> Dict[str, str]:
    cols = [c for c in num_df.columns if c != "patient_id"]
    out = {}
    # Prefer exact cleaned-table covariates.
    for name, col in EXACT_COVARIATE_COLUMNS.items():
        if col in cols:
            out[name] = col
...
def gradient_display_metric(variable: str, concept_df: Optional[pd.DataFrame] = None) -> Tuple[str, str, str]:
    ...
    # For continuous variables, use mean for display/trend plots. Medians can be
    # identical across quantile groups for integer-valued echo measures such as
    # LVPW_mm, producing a visually uninformative all-zero row after z-scoring.
    # Median/IQR are still saved in the output table.
    return "mean", "Mean", "original unit"
        ...
        for v in panel_vars:
            ...
            for g, sub in tmp.groupby("axis_group", observed=False):
                ...
                if n_non:
                    ...
                    std = float(np.nanstd(clean, ddof=1)) if n_non > 1 else 0.0
                    sem = float(std / np.sqrt(n_non)) if n_non > 1 else 0.0
                    q25 = float(np.nanpercentile(clean, 25))
                    q75 = float(np.nanpercentile(clean, 75))
                else:
                    prop_ge_1 = prop_ge_2 = prop_ge_3 = median = mean = std = sem = q25 = q75 = np.nan
                display_value = {
                    "mean": mean,
                    "median": median,
                    "prop_ge_1": prop_ge_1,
...
def plot_gradient_small_multiples(gradient_df: pd.DataFrame, out_path: Path, panel: str, component: int = 1, max_vars: int = 8) -> None:
    ...
    for ax, var in zip(axes.ravel(), variables):
        ...
        if sub["display_unit"].iloc[0] == "original unit" and "sem" in sub.columns:
            yerr = sub["sem"].to_numpy(float)
            if np.isfinite(yerr).any():
                ax.errorbar(x, y, yerr=yerr, marker="o", lw=2, capsize=3)
            else:
...
def plot_continuous_gradient_small_multiples(gradient_df: pd.DataFrame, out_path: Path, panel: str, component: int = 1, max_vars: int = 8) -> None:
    """Small-multiple trend plot for continuous clinical variables only.

    This is useful when the heatmap hides subtle changes after row-wise
    standardization or when integer-valued medians are identical across groups.
    Continuous variables are shown as group mean ± SEM.
    """
    d = gradient_df[(gradient_df["panel"] == panel) & (gradient_df["adjustment"] == "none") & (gradient_df["component"] == component)].copy()
    if len(d) == 0:
        return
    if "variable_kind" in d.columns:
        d = d[d["variable_kind"] == "continuous"].copy()
    if len(d) == 0:
        return
    rank = d.groupby("variable")["axis_variable_spearman"].first().abs().sort_values(ascending=False)
    variables = rank.head(max_vars).index.tolist()
    ...
    n_cols = 2 if n_vars > 3 else 1
    n_rows = int(np.ceil(n_vars / n_cols))
    fig, axes = plt.subplots(n_rows, n_cols, figsize=(5.2 * n_cols, 3.25 * n_rows), squeeze=False)
    x_order = [f"Q{i+1}" for i in range(d["axis_group"].nunique())]
    for ax, var in zip(axes.ravel(), variables):
        sub = d[d["variable"] == var].copy()
        sub["axis_group"] = pd.Categorical(sub["axis_group"], categories=x_order, ordered=True)
        sub = sub.sort_values("axis_group")
        x = np.arange(len(sub))
        y = sub["mean"].to_numpy(float) if "mean" in sub.columns else sub["display_value"].to_numpy(float)
        yerr = sub["sem"].to_numpy(float) if "sem" in sub.columns else np.full_like(y, np.nan, dtype=float)
        if np.isfinite(yerr).any():
            ax.errorbar(x, y, yerr=yerr, marker="o", lw=2, capsize=3)
        else:
            ax.plot(x, y, marker="o", lw=2)
        ax.set_xticks(x)
        ax.set_xticklabels(sub["axis_group"].astype(str).tolist())
        rho = sub["axis_variable_spearman"].iloc[0] if "axis_variable_spearman" in sub.columns else np.nan
        title = f"{var}" + (f" (ρ={rho:.2f})" if np.isfinite(rho) else "")
        ax.set_title(title)
        ax.set_ylabel("Mean ± SEM")
        ax.set_xlabel(f"CCA axis {component} quantile")
    for ax in axes.ravel()[n_vars:]:
        ax.axis("off")
    fig.suptitle(f"Continuous clinical gradients across {panel} CCA acoustic axis {component}", y=1.02, fontsize=16)
    savefig(out_path)
...
def main():
    ...
    for panel_name, vars_ in panels.items():
        ...
        if panel_name in DOMAIN_PLOT_PANELS:
            ...
            plot_continuous_gradient_small_multiples(grad, fig_dir / f"{clean_filename(panel_name)}_axis1_continuous_gradient_small_multiples.png", panel=panel_name, component=1)
