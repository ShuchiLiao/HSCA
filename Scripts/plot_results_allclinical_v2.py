...
4) endpoint ROC curves with CI bands, comparing axis 1 vs axis 1+2 when available.
...
ENDPOINT_FIGURES = [
    ("EF_lt_40", "EF < 40", "figure_06_roc_EF_lt_40_axis1_vs_axis12.png"),
    ("NTproBNP_ge_300", "NT-proBNP ≥ 300", "figure_07_roc_NTproBNP_ge_300_axis1_vs_axis12.png"),
    ("LVEDD_dilated", "LVEDD dilated", "figure_08_roc_LVEDD_dilated_axis1_vs_axis12.png"),
    ("NYHA_ge_3", "NYHA ≥ 3", "figure_09_roc_NYHA_ge_3_axis1_vs_axis12.png"),
]
...
def plot_figure03_selected_continuous_box_points(patient_long: pd.DataFrame, out_path: Path, component: int = 1) -> None:
    ...
    for ax, var in zip(axes.ravel(), vars_order):
        ...
        color = pal.get(VARIABLE_GROUP.get(var), "#0072B2")
        if HAS_SEABORN:
            sns.boxplot(
...
# 图五：Ordinal profile heatmap，仅 NYHA≥2 和 NYHA≥3。
def plot_figure05_nyha_heatmap(summary_df: pd.DataFrame, out_path: Path, component: int = 1) -> None:
    if len(summary_df) == 0:
        return
    required = {"panel", "adjustment", "component", "axis_group", "variable"}
    if not required.issubset(summary_df.columns):
        log(f"Skip figure05: missing {required - set(summary_df.columns)}")
        return
    d = summary_df[
        summary_df["panel"].astype(str).eq("all_clinical")
        & summary_df["adjustment"].astype(str).eq("none")
        & summary_df["component"].astype(int).eq(int(component))
        & summary_df["variable"].astype(str).eq("NYHA")
    ].copy()
    if len(d) == 0:
        return
    rows = []
    for label, col in [("NYHA ≥2", "prop_ge_2"), ("NYHA ≥3", "prop_ge_3")]:
        if col not in d.columns:
            continue
        rec = {"row_label": label}
        for q in ["Q1", "Q2", "Q3", "Q4"]:
            hit = d[d["axis_group"].astype(str).eq(q)]
    ...
    for tick in ax.get_yticklabels():
        tick.set_color(domain_palette()["Function"])
    ax.set_xlabel("All-clinical acoustic axis 1 quartile")
    ax.set_ylabel("")
    ax.set_title("NYHA burden across all-clinical acoustic-axis quartiles")
    savefig(out_path)


# 图六至图九：Endpoint ROC 曲线，带 bootstrap CI，比较 Axis 1 和 Axis 1+2。
def plot_endpoint_roc(pred_df: pd.DataFrame, summary_df: pd.DataFrame, endpoint: str, pretty: str, out_path: Path, args) -> None:
    ...
    for k in [1, 2]:
        ...
        row = summary[(summary["endpoint"].astype(str) == endpoint) & (summary["n_axis_features"].astype(int) == k)] if len(summary) else pd.DataFrame()
        if len(row):
        ...
        if np.isfinite(lo).any():
            ax.fill_between(fpr, lo, hi, color=colors[k], alpha=0.14)
        lab = f"{labels[k]}: AUROC={auc:.3f}"
        if np.isfinite(auc_lo) and np.isfinite(auc_hi):
            lab += f" ({auc_lo:.3f}–{auc_hi:.3f})"
        ax.plot(fpr, tpr, lw=2.4, color=colors[k], label=lab)
        plotted = True
    if not plotted:
        plt.close()
        return
...
def plot_all_results(table_dir: Path, fig_dir: Path, args) -> None:
    ...
    if len(gradient_summary):
        plot_figure05_nyha_heatmap(
            gradient_summary,
            fig_dir / "figure_05_allclinical_axis1_NYHA_ge2_ge3_heatmap.png",
            component=1,
        )
