...
V2 script for the manuscript question:
...
1) Single-variable acoustic readout is used as a complementary signal overview
   and as one input for core-panel selection. It is NOT framed as a failed
   alternative to multivariable alignment.
...
3) Clinical panels are analyzed separately:
   - function_burden: EF, NT-proBNP, NYHA, heart rate, etc.
   - structure: LA, LVEDD, LVESD, IVS, LVPW, etc.
   - valve: MR, TR, AR, PR, AS, MS, etc.
   - all_clinical: all detected clinically relevant variables
   - core: selected from clinical importance + single-variable acoustic readout
...
Example
-------
python run_clinically_anchored_acoustic_phenotyping_v2.py --embedding-dir Representation_learning/embeddings_4_1/beats --clinical-csv Clinical_alignment/outputs/prepared/aligned_clinical_clean.csv --out-dir Clinical_alignment/outputs/clinically_anchored_acoustic_phenotyping_v2 --n-splits 5 --n-components 2 --n-pca 50 --n-permutations 100 --n-bootstrap 1000 --n-loading-bootstrap 200 --n-random-controls 20 --seed 42
"""
...
CONCEPTS: List[ClinicalConcept] = [
    ...
    ClinicalConcept("HR", "function_burden", [r"^hr$", r"heart.*rate", r"心率"], +1, 2),
    ...
    ClinicalConcept("MR", "valve", [r"^mr$", r"mitral.*regurg", r"二尖瓣.*反流"], +1, 1),
    ClinicalConcept("TR", "valve", [r"^tr$", r"tricuspid.*regurg", r"三尖瓣.*反流"], +1, 1),
    ClinicalConcept("AR", "valve", [r"^ar$", r"aortic.*regurg", r"主动脉.*反流"], +1, 2),
    ClinicalConcept("PR", "valve", [r"^pr$", r"pulmonary.*regurg", r"肺动脉.*反流"], +1, 3),
    ClinicalConcept("AS", "valve", [r"^as$", r"aortic.*sten", r"主动脉.*狭窄"], +1, 2),
    ClinicalConcept("MS", "valve", [r"^ms$", r"mitral.*sten", r"二尖瓣.*狭窄"], +1, 2),
]

DEMOGRAPHIC_PATTERNS = {
    "age": [r"^age$", r"年龄"],
    "sex": [r"^sex$", r"gender", r"性别", r"男=1", r"男"],
}
...
def detect_clinical_concepts(num_df: pd.DataFrame, args) -> pd.DataFrame:
    ...
    rows = []
    for cc in CONCEPTS:
        col = select_first_matching_column([c for c in cols if c not in used_cols], cc.patterns)
        if col is None:
            rows.append({
                "concept": cc.concept, "domain": cc.domain, "column": "", "status": "not_found",
                "missing_fraction": np.nan, "n_nonmissing": 0, "n_unique": 0,
        ...
        status = "ok" if (miss <= args.max_missing and n_unique >= 2 and n_non >= args.min_target_n) else "filtered"
        if status == "ok":
            used_cols.add(col)
        rows.append({
            "concept": cc.concept, "domain": cc.domain, "column": col, "status": status,
            "missing_fraction": miss, "n_nonmissing": n_non, "n_unique": n_unique,
            "orientation": cc.orientation, "priority": cc.priority,
        })
...
def build_clinical_panels(concept_df: pd.DataFrame, single_df: pd.DataFrame, args) -> Tuple[Dict[str, List[str]], pd.DataFrame]:
    ...
    function_vars = get_ok_vars_by_domain(concept_df, "function_burden")
    structure_vars = get_ok_vars_by_domain(concept_df, "structure")
    valve_vars = get_ok_vars_by_domain(concept_df, "valve")
    all_vars = []
    for vs in [function_vars, structure_vars, valve_vars]:
        for v in vs:
            if v not in all_vars:
                all_vars.append(v)

    if len(function_vars) >= 2:
        panels["function_burden"] = function_vars
    if len(structure_vars) >= 2:
        panels["structure"] = structure_vars
    if len(valve_vars) >= 2:
        panels["valve"] = valve_vars
    if len(all_vars) >= 2:
        panels["all_clinical"] = all_vars

    # Core panel: clinical priority first, then add strongest single-variable readouts.
    priority_concepts = ["NTproBNP", "EF", "NYHA", "HR", "LVEDD", "LA", "MR", "TR"]
    core_vars: List[str] = []
    ...
    for concept in priority_concepts:
        ...
        if len(m):
            ...
            if col and col not in core_vars:
                core_vars.append(col)

    if single_df is not None and len(single_df) and "spearman_pred_true" in single_df.columns:
        s = single_df[single_df.get("status", "ok") == "ok"].copy()
        s = s[np.isfinite(s["spearman_pred_true"])]
        s["abs_rho"] = s["spearman_pred_true"].abs()
        s = s.sort_values("abs_rho", ascending=False)
        for col in s["variable"].tolist():
            if col in all_vars and col not in core_vars:
                core_vars.append(col)
            if len(core_vars) >= args.core_max_vars:
                break

    # If still too few, fill by clinical priority from all_vars.
    for col in all_vars:
        if col not in core_vars:
            core_vars.append(col)
        if len(core_vars) >= args.core_max_vars:
            break
    core_vars = core_vars[:args.core_max_vars]
    if len(core_vars) >= 2:
        # Put core first in output ordering.
        panels = {"core": core_vars, **panels}
...
def clinical_gradient_by_axis(score_df: pd.DataFrame, clinical: pd.DataFrame, panel_vars: List[str], args) -> pd.DataFrame:
    rows = []
    for comp in range(1, args.n_components + 1):
        xcol = f"cca_acoustic_axis{comp}"
        if xcol not in score_df.columns:
        ...
        for v in panel_vars:
            ...
            for g, sub in tmp.groupby("axis_group", observed=False):
                ...
                rows.append({
                    ...
                    "n": int(vv.notna().sum()),
                    "median": float(np.nanmedian(vv)) if vv.notna().sum() else np.nan,
                    "mean": float(np.nanmean(vv)) if vv.notna().sum() else np.nan,
                    "q25": float(np.nanpercentile(vv.dropna(), 25)) if vv.notna().sum() else np.nan,
                    "q75": float(np.nanpercentile(vv.dropna(), 75)) if vv.notna().sum() else np.nan,
...
def make_endpoint_series(name: str, col: str, clinical: pd.DataFrame) -> Tuple[pd.Series, float, str]:
    ...
    if name == "EF_lt_40":
        ...
        return (v < threshold).astype(float), threshold, f"{col} < {threshold:g}"
    if name == "NTproBNP_ge_300":
        ...
        if "log10" in lc or ("log" in lc and maxv <= 5.0):
            ...
            threshold = 300.0
        return (v >= threshold).astype(float), threshold, f"{col} >= {threshold:g} (corresponds to NT-proBNP >=300 if log-transformed)"
    if name == "NYHA_gt_3":
        threshold = 3.0
        return (v > threshold).astype(float), threshold, f"{col} > 3"
...
def run_endpoint_validation_core(X: np.ndarray, clinical: pd.DataFrame, patient_ids: Sequence[str], core_vars: List[str], args, concept_df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    log("Endpoint validation using CCA acoustic axes 1+2 and a simple logistic classifier")
    ...
    for endpoint_name, endpoint_col in endpoints.items():
        ...
        if len(y_all) < args.min_endpoint_n or len(np.unique(y_all)) < 2 or min(y_all.sum(), len(y_all) - y_all.sum()) < args.min_endpoint_class_n:
            info["status"] = "skipped_too_few_or_imbalanced"
            info_rows.append(info)
            log(f"  Skip {endpoint_name}: {info['status']}, n={len(y_all)}, pos={int(y_all.sum()) if len(y_all) else 0}")
            continue
        info["status"] = "ok"
        info_rows.append(info)
        X_e = X[valid.values]
        ...
        if args.leave_endpoint_out:
            panel_vars = endpoint_exclude_vars(endpoint_name, endpoint_col, core_vars)
            panel_type = "core_leave_endpoint_out"
            if len(panel_vars) < 2:
                panel_vars = core_vars
                panel_type = "core_full_fallback"
        else:
            panel_vars = core_vars
            panel_type = "core_full"
        ...
        summary_rows.append({
            ...
            "panel": "core", "panel_type": panel_type, "n_axis_features": 2,
...
def run_confounder_controls(X: np.ndarray, clinical: pd.DataFrame, patient_ids: Sequence[str], core_vars: List[str], cov_cols: Dict[str, str], args, concept_df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame]:
    log("Confounder controls for the core panel")
    rows = []
    assoc_rows = []
    analyses = []
    age_sex = [cov_cols[k] for k in ["age", "sex"] if k in cov_cols]
    if len(age_sex) > 0:
        analyses.append(("age_sex_residualized", core_vars, age_sex))
    age_sex_hr = [cov_cols[k] for k in ["age", "sex", "heart_rate"] if k in cov_cols]
    if len(age_sex_hr) > 0:
        core_no_hr = remove_cols_by_patterns(core_vars, [r"^hr$", r"heart.*rate", r"心率"])
        if len(core_no_hr) >= 2:
            analyses.append(("age_sex_heart_rate_residualized", core_no_hr, age_sex_hr))
    for label, pvars, cvars in analyses:
        try:
            Xp, Cp, pids_p, _ = filter_for_panel(X, clinical, patient_ids, pvars, args)
            sdf, fdf = run_oof_cca_panel(Xp, Cp, pvars, pids_p, "core", args, concept_df, covariate_cols=cvars, adjustment=label, verbose=True)
...
def run_leave_one_position_control(X: np.ndarray, clinical: pd.DataFrame, patient_ids: Sequence[str], core_vars: List[str], args, concept_df: pd.DataFrame) -> pd.DataFrame:
    ...
    for leave in positions:
        ...
        try:
            Xf, Cf, pids_f, _ = filter_for_panel(Xl, Cl, pids_l, core_vars, args)
            sdf, _ = run_oof_cca_panel(Xf, Cf, core_vars, pids_f, f"core_leave_{leave}", args, concept_df, verbose=True)
            summ = summarize_alignment(sdf, core_vars, Cf, args, seed_offset=3000)
...
def plot_single_variable_readout(df: pd.DataFrame, out_path: Path, top_n: int = 20) -> None:
    ...
    plt.figure(figsize=(9.5, max(5, 0.38 * len(d))))
    ax = plt.gca()
...
def plot_panel_alignment(df: pd.DataFrame, out_path: Path) -> None:
    ...
    d = d.sort_values(["panel", "component"])
    plt.figure(figsize=(9.5, max(5, 0.45 * len(d))))
    ax = plt.gca()
    y = np.arange(len(d))
    ...
    ax.set_title("CCA alignment across clinical panels")
    savefig(out_path)
...
def plot_negative_controls(ctrl: pd.DataFrame, out_path: Path, panel: str = "core") -> None:
    if len(ctrl) == 0:
        return
...
def write_report(out_dir: Path, config: Dict, panels: Dict[str, List[str]], concept_df: pd.DataFrame, single_df: pd.DataFrame,
    ...
    lines = []
    lines.append("# Clinically anchored acoustic phenotyping V2 summary\n\n")
    ...
    lines.append(concept_df[["concept", "domain", "column", "status", "missing_fraction", "n_nonmissing", "n_unique"]].pipe(df_to_markdown_safe, index=False))
    ...
    if len(assoc_df):
        lines.append("\n## Core axis interpretation\n")
        sub = assoc_df[(assoc_df["panel"] == "core") & (assoc_df["adjustment"] == "none") & (assoc_df["component"] == 1)].copy()
        if len(sub):
...
def parse_args():
    p = argparse.ArgumentParser(description="Clinically anchored acoustic phenotyping V2: CCA-only panel analyses")
    ...
    p.add_argument("--out-dir", type=str, default="Clinical_alignment/outputs/clinically_anchored_acoustic_phenotyping_v2")
...
def main():
    ...
    candidate_vars = concept_df.loc[concept_df["status"] == "ok", "column"].dropna().astype(str).tolist()
    if len(candidate_vars) < 2:
        raise ValueError("Too few clinically relevant candidate variables were detected. Check column names or --max-missing.")
    ...
    plot_single_variable_readout(single_df, fig_dir / "single_variable_readout_overview.png")

    panels, panel_vars_df = build_clinical_panels(concept_df, single_df, args)
    if "core" not in panels:
        raise ValueError("Core panel could not be built. Check clinical variables and missingness.")
    ...
    for panel_name, vars_ in panels.items():
        ...
        # Random embedding control only for core panel, to avoid excessive runtime.
        if panel_name == "core":
        ...
        grad = clinical_gradient_by_axis(sdf, Cp, vars_, args)
    ...
    plot_negative_controls(neg_ctrl_all, fig_dir / "negative_controls_core.png", panel="core")

    # Endpoint validation on core panel.
    core_vars = panels["core"]
    try:
        Xcore, Ccore, pids_core, _ = filter_for_panel(X, clinical, patient_ids, core_vars, args)
        endpoint_summary, endpoint_pred, endpoint_info = run_endpoint_validation_core(Xcore, Ccore, pids_core, core_vars, args, concept_df)
    except Exception as e:
    ...
    # Confounder controls on core panel.
    try:
        conf_summary, conf_assoc = run_confounder_controls(Xcore, Ccore, pids_core, core_vars, cov_cols, args, concept_df)
    except Exception as e:
    ...
    # Leave-one-position-out position coverage control.
    try:
        lopo_summary = run_leave_one_position_control(X, clinical, patient_ids, core_vars, args, concept_df)
    except Exception as e:
